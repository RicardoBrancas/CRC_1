...
def plfit(x, *varargin):
        ...
        if type(varargin[i])==str: 
            if varargin[i]=='range':
                ...
                if Range[1]>Range[0]:
                    ...
                    vec=[]
                try:
                    vec=[X*float(Range[2])+Range[0] for X in range(int(old_div((Range[1]-Range[0]),Range[2])))]
    ...
    if f_dattype== 'REAL':
        ...
        if sample!=[]:
            step = old_div(float(len(xmins)),(sample-1))
            index_curr=0
            new_xmins=[]
        ...
        for xm in range(0,len(xmins)):
            ...
            a    = old_div(float(n), sum([log(old_div(float(X),xmin)) for X in z]))
            if nosmall:
                if old_div((a-1),sqrt(n)) > 0.1 and dat!=[]:
                    xm = len(xmins)+1
                    break
            ...
            cf   = [1-pow((old_div(float(xmin),X)),a) for X in z]
            dat.append( max( [abs(cf[X]-old_div(float(X),n)) for X in range(0,n)]))
        D     = min(dat)
        xmin  = xmins[dat.index(D)]
        z     = [X for X in x if X>=xmin]
        z.sort()
        n     = len(z) 
        alpha = 1 + old_div(n, sum([log(old_div(float(X),xmin)) for X in z]))
        if finite: alpha = alpha*float(n-1)/n+old_div(1.,n)  # finite-size correction
        if n < 50 and not finite and not nowarn:
            print('(PLFIT) Warning: finite-size bias may be present.\n')
        
        L = n*log(old_div((alpha-1),xmin)) - alpha*sum([log(old_div(float(X),xmin)) for X in z])
    elif f_dattype== 'INTS':
        
        x=list(map(int,x))
        if vec==[]:
            for X in range(150,351):
                vec.append(old_div(X,100.))    # covers range of most practical 
                                    # scaling parameters
        ...
        for xm in range(0,len(xmins)):
            ...
            fit = reduce(lambda X,Y: X+[Y+X[-1]],\
                         ([old_div(pow(X,-vec[I]),(float(zvec[I])-sum([pow(X,-vec[I]) for X in list(map(float,list(range(1,xmin))))]))) for X in range(xmin,xmax+1)]),[0])[1:]
            cdi=[]
            for XM in range(xmin,xmax+1):
                cdi.append(old_div(len([X for X in z if floor(X)<=XM]),float(n)))
        ...
        n     = len(z)
        alpha = datB[I]
        if finite: alpha = alpha*(n-1.)/n+old_div(1.,n)  # finite-size correction
        if n < 50 and not finite and not nowarn:
            print('(PLFIT) Warning: finite-size bias may be present.\n')
...
def zeta(s):
    ...
    if s <= 2.0:
        if s <= 1.0:
            return old_div(_polyval(_zeta_0,s),(s-1))
        return old_div(_polyval(_zeta_1,s),(s-1))
    z = old_div(_polyval(_zeta_P,s), _polyval(_zeta_Q,s))
